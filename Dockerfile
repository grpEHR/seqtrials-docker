...
RUN <<EOF
    ...
    echo 'options(repos = c(CRAN = sprintf("https://packagemanager.posit.co/cran/'"${CRAN_DATE}"'/bin/linux/manylinux_2_28-%s/%s", R.version["arch"], substr(getRversion(), 1, 3))))' > ~/.Rprofile
